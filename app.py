...
if page == "📊 Panel Efficiency Analyzer":
    ...
    # Page Layout Columns
    col_input, col_display = tf.columns([1, 2], gap="large")
...
# ==========================================
    ...
    c_in, c_out = tf.columns([1, 2], gap="large")
    ...
    if monthly_solar_production_kwh >= monthly_consumption_kwh:
        # Array overproduces: grid consumption fully offset + net metering compensation
        excess_kwh = monthly_solar_production_kwh - monthly_consumption_kwh
        monthly_savings = monthly_bill + (excess_kwh * buyback_rate)
    else:
        # Array underproduces: grid covers deficit at full cost
    ...
    cash_flows_chart_data = [{"Year": 0, "Net Cash Flow ($)": -install_cost, "Balance": "Negative Balance"}]
    
    for year in range(1, 21):
        # Apply slight annual degradation to output calculations (0.5%/yr)
        ...
        balance_state = "Investment Recovered" if cumulative_cash_flow >= 0 else "Negative Balance"
        cash_flows_chart_data.append({
            "Year": year, 
            "Net Cash Flow ($)": cumulative_cash_flow,
    ...
    with c_out:
        tf.subheader("📊 Capital Projections")
        
        f1, f2, f3 = tf.columns(3)
        if payback_period <= 20:
            f1.metric("Payback Period", f"{payback_period:.1f} Years", "Excellent Return" if payback_period < 7 else "Standard Return")
        else:
            f1.metric("Payback Period", "> 20 Years", "Long Return Horizon", delta_color="inverse")
            
        f2.metric("20-Yr Cumulative Savings", f"${cumulative_cash_flow:,.0f}")
        f3.metric("Net Present Value (NPV)", f"${npv:,.2f}", "5% Discount Rate Basis", delta_color="off")
        
        tf.markdown("---")
        
        # Financial Horizon Plotly Bar Graph
        fig_finance = px.bar(
            df_finance, 
            x="Year", 
            y="Net Cash Flow ($)", 
            color="Balance",
            color_discrete_map={"Negative Balance": "#FF4B4B", "Investment Recovered": "#00C04B"},
            title="20-Year Cumulative Capital Recovery Curve"
        )
        
        # Horizontal reference baseline marking net-zero break even point
